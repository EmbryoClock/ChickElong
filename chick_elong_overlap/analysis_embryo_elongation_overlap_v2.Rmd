...
```{r distrib_overlap_plots, message=FALSE, warning=FALSE}
...
```{r distrib_overlap_time, message=FALSE, warning=FALSE}
...
overlap_plots_time <- setNames(
  lapply(names(clean_time_list), function(name) {
    calculate_pairwise_overlap(clean_time_list[[name]], name)
  }),
  names(clean_time_list)
)

# Print the plots
combined_plots_time <- lapply(overlap_plots_time, function(sublist) {
  patchwork::wrap_plots(sublist, ncol = 2)  # Combines the plots at this level
})

print(combined_plots_time)
